...
type RoomResponse = RoomSuccess | RoomError;

interface RoomHeaderUserProps {
  room: RoomResponse
}

export default function RoomHeader({ room }: RoomHeaderUserProps) {
  ...
  if (!room) {
    return <div>Loading ....Please Wait a Second !!</div>;
  }

  if (!room.success) {
    return <div>{room.error}</div>;
  }
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="px-2 py-0.5 bg-brand/20 text-brand-light text-[10px] font-black uppercase rounded border border-brand/20">
            LIVE
          </span>
        </div>
        <div className="flex items-center gap-4 text-sm font-medium text-white/40">
          <p>
            Hosted by <span className="text-white">{room.hostName}</span>
          </p>
